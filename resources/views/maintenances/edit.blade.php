...
<div class="py-4 animate-fade-in">
    <div class="content-card">
        <div class="card-header-modern">
            ...
            <h4>Modifier la Maintenance</h4>
            <p class="text-white-50 mb-0 mt-2">Maintenance #{{ $maintenance->id }}</p>
        </div>
        
        <div class="card-body p-4 p-md-5">
            <form action="{{ route('maintenances.update', $maintenance) }}" method="POST" class="form-modern">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="truck_id" class="form-select @error('truck_id') is-invalid @enderror">
                        @foreach($trucks as $truck)
                            <option value="{{ $truck->id }}" {{ old('truck_id', $maintenance->truck_id) == $truck->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $maintenance->date) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="cost" step="0.01" class="form-control @error('cost') is-invalid @enderror" value="{{ old('cost', $maintenance->cost) }}" placeholder="Ex: 1500.00">
                    @error('cost') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-4">
                    <label class="form-label"><i class="bi bi-card-text me-1"></i> Description</label>
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="Détails supplémentaires...">{{ old('description', $maintenance->description) }}</textarea>
