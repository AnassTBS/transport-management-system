...
<div class="py-4 animate-fade-in">
    <div class="content-card">
        <div class="card-header-modern">
            ...
            <p class="text-white-50 mb-0 mt-2">Enregistrer une intervention sur un véhicule</p>
        </div>
        
        <div class="card-body p-4 p-md-5">
            <form action="{{ route('maintenances.store') }}" method="POST" class="form-modern">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label class="form-label"><i class="bi bi-gear me-1"></i> Type d'intervention</label>
                        <select name="type" class="form-select @error('type') is-invalid @enderror">
                            <option value="vidange" {{ old('type') == 'vidange' ? 'selected' : '' }}>Vidange</option>
                            <option value="pneus" {{ old('type') == 'pneus' ? 'selected' : '' }}>Pneus</option>
                            <option value="freins" {{ old('type') == 'freins' ? 'selected' : '' }}>Freins</option>
                            <option value="moteur" {{ old('type') == 'moteur' ? 'selected' : '' }}>Moteur</option>
                ...
                <div class="mb-4">
                    <label class="form-label"><i class="bi bi-cash me-1"></i> Coût (MAD)</label>
                    <input type="number" name="cost" step="0.01" class="form-control @error('cost') is-invalid @enderror" value="{{ old('cost') }}" placeholder="Ex: 1500.00">
                    @error('cost') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-4">
                    <label class="form-label"><i class="bi bi-card-text me-1"></i> Description</label>
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="Détails supplémentaires sur l'intervention...">{{ old('description') }}</textarea>
                ...
                <div class="d-flex gap-3 pt-3 mt-2 border-top">
                    ...
                    <a href="{{ route('maintenances.index') }}" class="btn-cancel text-decoration-none text-center px-4">
                        Annuler
                    </a>
                </div>
            </form>
