...
<div class="page-header-modern animate-fade-in d-flex justify-content-between align-items-center flex-wrap gap-3">
    <div class="page-header-content">
        <h2 class="mb-1 fw-bold">Gestion des Livraisons</h2>
        <p class="mb-0 text-white-50"><i class="bi bi-box-seam me-2"></i>{{ $deliveries->total() }} livraison(s) enregistrée(s)</p>
    </div>
    <div class="page-header-content">
        <a href="{{ route('deliveries.create') }}" class="btn-modern text-decoration-none">
        ...
        <table class="table-modern">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="text-muted"><i class="bi bi-calendar-event me-1"></i>{{ \Carbon\Carbon::parse($delivery->date_depart)->format('d/m/Y') }}</span></td>
                    <td>
                        @if($delivery->date_arrivee)
                            <span class="text-muted"><i class="bi bi-calendar-check me-1"></i>{{ \Carbon\Carbon::parse($delivery->date_arrivee)->format('d/m/Y') }}</span>
                        @else
                            <span class="badge bg-light text-secondary border border-secondary-subtle px-2"><i class="bi bi-hourglass-split me-1"></i>En attente</span>
                        @endif
                    </td>
                    <td>
                        <span class="text-muted small">
                            <i class="bi bi-person-badge me-1"></i>
                            {{ $delivery->user->name ?? 'Système' }}
                        </span>
                    </td>
                    <td>
                        @php
                            $badgeData = match($delivery->status) {
                                'en_cours' => ['class' => 'badge-primary', 'icon' => 'bi-arrow-repeat'],
                        ...
                        <span class="modern-badge {{ $badgeData['class'] }}">
                            <i class="bi {{ $badgeData['icon'] }}"></i> {{ ucfirst(str_replace('_', ' ', $delivery->status)) }}
                        </span>
                    </td>
                    <td class="text-end pe-4">
                        <a href="{{ route('deliveries.edit', $delivery) }}" class="btn-action btn-edit me-1" title="Modifier">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('deliveries.destroy', $delivery) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette livraison ?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="7" class="text-center text-muted py-5">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            <i class="bi bi-box-seam fs-1 text-black-50 mb-3"></i>
                            <h5>Aucune livraison enregistrée</h5>
                            <p class="text-muted">Commencez par planifier une nouvelle livraison.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
