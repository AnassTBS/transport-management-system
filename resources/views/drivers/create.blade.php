...
<div class="py-4 animate-fade-in">
    <div class="content-card">
        <div class="card-header-modern">
            ...
            <h4>Ajouter un Nouveau Chauffeur</h4>
            <p class="text-white-50 mb-0 mt-2">Veuillez renseigner les informations du conducteur</p>
        </div>
        
        <div class="card-body p-4 p-md-5">
            <form action="{{ route('drivers.store') }}" method="POST" class="form-modern">
                @csrf

                <div class="mb-4">
                    <label class="form-label"><i class="bi bi-person me-1"></i> Nom complet</label>
                    <input type="text" name="nom" class="form-control @error('nom') is-invalid @enderror" value="{{ old('nom') }}" placeholder="ex: Hassan Alaoui">
                    @error('nom') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-4">
                    <label class="form-label"><i class="bi bi-telephone me-1"></i> Téléphone</label>
                    <input type="text" name="tel" class="form-control @error('tel') is-invalid @enderror" value="{{ old('tel') }}" placeholder="ex: 06XXXXXXXX">
                    @error('tel') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label class="form-label"><i class="bi bi-card-heading me-1"></i> N° Permis</label>
                        <input type="text" name="permis" class="form-control @error('permis') is-invalid @enderror" value="{{ old('permis') }}" placeholder="ex: D-001">
                ...
                <div class="d-flex gap-3 pt-3 mt-2 border-top">
                    ...
                    <a href="{{ route('drivers.index') }}" class="btn-cancel text-decoration-none text-center px-4">
                        Annuler
                    </a>
                </div>
            </form>
