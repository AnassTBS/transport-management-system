...
<div class="page-header-modern animate-fade-in d-flex justify-content-between align-items-center flex-wrap gap-3">
    <div class="page-header-content">
        <h2 class="mb-1 fw-bold">Gestion des Chauffeurs</h2>
        <p class="mb-0 text-white-50"><i class="bi bi-person-badge me-2"></i>{{ $drivers->total() }} chauffeur(s) enregistré(s) dans la flotte</p>
    </div>
    <div class="page-header-content">
        <a href="{{ route('drivers.create') }}" class="btn-modern text-decoration-none">
        ...
        <table class="table-modern">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $driver->tel }}</td>
                    <td><span class="text-muted">{{ $driver->permis }}</span></td>
                    <td>
                        @php
                            $badgeData = match($driver->status) {
                                'actif'   => ['class' => 'badge-success', 'icon' => 'bi-check-circle-fill'],
                                'inactif' => ['class' => 'badge-secondary', 'icon' => 'bi-x-circle-fill'],
                                default   => ['class' => 'badge-secondary', 'icon' => 'bi-info-circle-fill'],
                            };
                        @endphp
                        <span class="modern-badge {{ $badgeData['class'] }}">
                            <i class="bi {{ $badgeData['icon'] }}"></i> {{ ucfirst($driver->status) }}
                        </span>
                    </td>
                    <td class="text-end pe-4">
                        <a href="{{ route('drivers.edit', $driver) }}" class="btn-action btn-edit me-1" title="Modifier">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('drivers.destroy', $driver) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer ce chauffeur ?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="5" class="text-center text-muted py-5">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            <i class="bi bi-people fs-1 text-black-50 mb-3"></i>
                            <h5>Aucun chauffeur enregistré</h5>
                            <p class="text-muted">Commencez par ajouter un nouveau chauffeur à la flotte.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
